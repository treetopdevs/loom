defmodule Loom.Teams.Cluster do
  @moduledoc """
  ...
  Only active when `config :loom, :cluster, enabled: true`.
  ...
  def enabled? do
    Application.get_env(:loom, :cluster, [])
  ...
  def handle_node_join(node) do
    ...
    Phoenix.PubSub.broadcast(Loom.PubSub, "cluster:events", {:node_joined, node})
    :ok
  end
  ...
  def handle_node_leave(node) do
    ...
    Phoenix.PubSub.broadcast(Loom.PubSub, "cluster:events", {:node_left, node})
    :ok
  end
