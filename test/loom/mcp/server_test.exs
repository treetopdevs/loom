defmodule Loom.MCP.ServerTest do
  use ExUnit.Case, async: true

  alias Loom.MCP.Server

  describe "module configuration" do
    test "publishes all 11 Loom tools" do
      ...
      assert Loom.Tools.FileRead in tool_modules
      assert Loom.Tools.FileWrite in tool_modules
      assert Loom.Tools.FileEdit in tool_modules
      assert Loom.Tools.FileSearch in tool_modules
      assert Loom.Tools.ContentSearch in tool_modules
      assert Loom.Tools.DirectoryList in tool_modules
      assert Loom.Tools.Shell in tool_modules
      assert Loom.Tools.Git in tool_modules
      assert Loom.Tools.DecisionLog in tool_modules
      assert Loom.Tools.DecisionQuery in tool_modules
      assert Loom.Tools.SubAgent in tool_modules
    end
  ...
  describe "enabled?/0" do
    ...
    test "returns true when server_enabled is true in config" do
      Loom.Config.put(:mcp, %{server_enabled: true})
    ...
    test "returns false when server_enabled is false" do
      Loom.Config.put(:mcp, %{server_enabled: false})
      refute Server.enabled?()
      # Clean up
      Loom.Config.put(:mcp, nil)
    end
