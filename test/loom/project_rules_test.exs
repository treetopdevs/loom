defmodule Loom.ProjectRulesTest do
  use ExUnit.Case, async: true

  alias Loom.ProjectRules

  describe "load/1" do
    @tag :tmp_dir
    test "returns empty rules when no LOOM.md exists", %{tmp_dir: tmp_dir} do
    ...
    @tag :tmp_dir
    test "loads LOOM.md file", %{tmp_dir: tmp_dir} do
      File.write!(Path.join(tmp_dir, "LOOM.md"), "Hello world")
      assert {:ok, rules} = ProjectRules.load(tmp_dir)
      assert rules.raw == "Hello world"
    end

    @tag :tmp_dir
    test "loads .loom.md file as fallback", %{tmp_dir: tmp_dir} do
      File.write!(Path.join(tmp_dir, ".loom.md"), "Hidden rules")
      assert {:ok, rules} = ProjectRules.load(tmp_dir)
      assert rules.raw == "Hidden rules"
    end

    @tag :tmp_dir
    test "prefers LOOM.md over .loom.md", %{tmp_dir: tmp_dir} do
      File.write!(Path.join(tmp_dir, "LOOM.md"), "Primary")
      File.write!(Path.join(tmp_dir, ".loom.md"), "Secondary")
      assert {:ok, rules} = ProjectRules.load(tmp_dir)
    ...
    test "parses rules section", %{tmp_dir: tmp_dir} do
      ...
      * Prefer pipes over nesting
      ...
      File.write!(Path.join(tmp_dir, "LOOM.md"), content)
      assert {:ok, rules} = ProjectRules.load(tmp_dir)
    ...
    test "handles full document with all sections", %{tmp_dir: tmp_dir} do
      content = """
      You are a coding assistant for the Loom project.

      ## Context
      Loom is an AI coding agent built in Elixir.
      ...
      assert rules.instructions =~ "Loom is an AI coding agent"
  ...
  describe "find_rules_file/1" do
    ...
    @tag :tmp_dir
    test "finds LOOM.md", %{tmp_dir: tmp_dir} do
      path = Path.join(tmp_dir, "LOOM.md")
      File.write!(path, "")
      assert ProjectRules.find_rules_file(tmp_dir) == path
    end

    @tag :tmp_dir
    test "finds .loom.md when LOOM.md absent", %{tmp_dir: tmp_dir} do
      path = Path.join(tmp_dir, ".loom.md")
      File.write!(path, "")
      assert ProjectRules.find_rules_file(tmp_dir) == path
    end
