defmodule Loom.Teams.ClusterTest do
  use ExUnit.Case, async: false

  alias Loom.Teams.{Cluster, Distributed, Migration}
  ...
  describe "Cluster.enabled?/0" do
    ...
    test "returns true when configured" do
      original = Application.get_env(:loom, :cluster)
      Application.put_env(:loom, :cluster, enabled: true)

      assert Cluster.enabled?()

      # Restore
      if original, do: Application.put_env(:loom, :cluster, original), else: Application.delete_env(:loom, :cluster)
    end
  end
  ...
  describe "Cluster.handle_node_join/1" do
    test "broadcasts node_joined event" do
      Phoenix.PubSub.subscribe(Loom.PubSub, "cluster:events")
  ...
  describe "Distributed.active_supervisor/0" do
    test "returns local supervisor when clustering is off" do
      assert Distributed.active_supervisor() == Loom.Teams.AgentSupervisor
    end
  end

  describe "Distributed.active_registry/0" do
    test "returns local registry when clustering is off" do
      assert Distributed.active_registry() == Loom.Teams.AgentRegistry
    end
  end
