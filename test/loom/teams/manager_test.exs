defmodule Loom.Teams.ManagerTest do
  ...
  describe "create_team/1" do
    test "returns {:ok, team_id} and creates ETS table" do
      ...
      assert {:ok, ref} = Loom.Teams.TableRegistry.get_table(team_id)
    ...
    test "stores metadata in ETS" do
      ...
      table = Loom.Teams.TableRegistry.get_table!(team_id)
  ...
  describe "dissolve_team/1" do
    test "cleans up ETS table" do
      ...
      {:ok, ref} = Loom.Teams.TableRegistry.get_table(team_id)
      assert :ets.info(ref) != :undefined

      assert :ok = Manager.dissolve_team(team_id)
      assert :ets.info(ref) == :undefined
      assert :error = Loom.Teams.TableRegistry.get_table(team_id)
    end
    ...
    test "resets RateLimiter budget" do
      ...
      Loom.Teams.RateLimiter.record_usage(team_id, "agent-1", %{tokens: 100, cost: 0.01})
      budget_before = Loom.Teams.RateLimiter.get_budget(team_id)
      assert budget_before.spent > 0

      Manager.dissolve_team(team_id)

      # After dissolution, budget should be reset (fresh state)
      budget_after = Loom.Teams.RateLimiter.get_budget(team_id)
