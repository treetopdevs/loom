defmodule Loom.Teams.RoleTest do
  use ExUnit.Case, async: true

  alias Loom.Teams.Role
  ...
  describe "tool scoping" do
    test "lead has all tools" do
      ...
      assert Loom.Tools.FileRead in lead.tools
      assert Loom.Tools.FileWrite in lead.tools
      assert Loom.Tools.FileEdit in lead.tools
      assert Loom.Tools.Shell in lead.tools
      assert Loom.Tools.Git in lead.tools
      assert Loom.Tools.SubAgent in lead.tools
      assert Loom.Tools.DecisionLog in lead.tools
      assert Loom.Tools.DecisionQuery in lead.tools
      assert Loom.Tools.LspDiagnostics in lead.tools
    end

    test "researcher has read-only and decision tools only" do
      {:ok, researcher} = Role.get(:researcher)
      assert Loom.Tools.FileRead in researcher.tools
      assert Loom.Tools.FileSearch in researcher.tools
      assert Loom.Tools.ContentSearch in researcher.tools
      assert Loom.Tools.DirectoryList in researcher.tools
      assert Loom.Tools.DecisionLog in researcher.tools
      assert Loom.Tools.DecisionQuery in researcher.tools

      refute Loom.Tools.FileWrite in researcher.tools
      refute Loom.Tools.FileEdit in researcher.tools
      refute Loom.Tools.Shell in researcher.tools
      refute Loom.Tools.Git in researcher.tools
    end

    test "coder has read, write, exec, and decision_log tools" do
      {:ok, coder} = Role.get(:coder)
      assert Loom.Tools.FileRead in coder.tools
      assert Loom.Tools.FileWrite in coder.tools
      assert Loom.Tools.FileEdit in coder.tools
      assert Loom.Tools.Shell in coder.tools
      assert Loom.Tools.Git in coder.tools
      assert Loom.Tools.DecisionLog in coder.tools

      refute Loom.Tools.SubAgent in coder.tools
      refute Loom.Tools.DecisionQuery in coder.tools
    end

    test "reviewer has read-only, shell, and decision tools" do
      {:ok, reviewer} = Role.get(:reviewer)
      assert Loom.Tools.FileRead in reviewer.tools
      assert Loom.Tools.Shell in reviewer.tools
      assert Loom.Tools.DecisionLog in reviewer.tools
      assert Loom.Tools.DecisionQuery in reviewer.tools

      refute Loom.Tools.FileWrite in reviewer.tools
      refute Loom.Tools.FileEdit in reviewer.tools
      refute Loom.Tools.Git in reviewer.tools
    end

    test "tester has read-only, shell, and decision_log" do
      {:ok, tester} = Role.get(:tester)
      assert Loom.Tools.FileRead in tester.tools
      assert Loom.Tools.Shell in tester.tools
      assert Loom.Tools.DecisionLog in tester.tools

      refute Loom.Tools.FileWrite in tester.tools
      refute Loom.Tools.FileEdit in tester.tools
      refute Loom.Tools.Git in tester.tools
      refute Loom.Tools.DecisionQuery in tester.tools
    end
  end
  ...
  describe "from_config/2" do
    ...
    test "resolves tool names from strings" do
      ...
      assert role.tools == [Loom.Tools.FileRead, Loom.Tools.Shell]
    end

    test "preserves tool modules passed directly" do
      config = %{
        tools: [Loom.Tools.FileRead, Loom.Tools.Git]
      }

      role = Role.from_config(:direct, config)

      assert role.tools == [Loom.Tools.FileRead, Loom.Tools.Git]
    end
  end
end
