defmodule Loom.Teams.TemplatesTest do
  use ExUnit.Case, async: false

  alias Loom.Teams.{Manager, Templates, TableRegistry}

  setup do
    # Store original teams config
    original = Loom.Config.get(:teams)

    on_exit(fn ->
      if original do
        Loom.Config.put(:teams, original)
      else
        # Remove if didn't exist
        Loom.Config.put(:teams, nil)
      end
    end)
  ...
  defp setup_templates(templates_map) do
    ...
    Loom.Config.put(:teams, teams_config)
  end
