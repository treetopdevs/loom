defmodule Loom.TelemetryTest do
  ...
  describe "span_llm_request/2" do
    test "emits start and stop events on success" do
      ...
      result =
        LoomTelemetry.span_llm_request(metadata, fn ->
          {:ok, %{}}
        end)

      assert result == {:ok, %{}}

      assert_received {:telemetry_event, [:loom, :llm, :request, :start], %{system_time: _},
                        %{session_id: "test-session", model: "anthropic:test"}}

      assert_received {:telemetry_event, [:loom, :llm, :request, :stop], %{duration: duration},
                        stop_meta}
    ...
    test "emits error metadata on failure" do
      ...
      assert_received {:telemetry_event, [:loom, :llm, :request, :stop], %{duration: _},
                        %{error: true}}
    end
  end
  ...
  describe "span_tool_execute/2" do
    test "emits start and stop events with success flag" do
      ...
      result =
        LoomTelemetry.span_tool_execute(metadata, fn ->
          {:ok, "file contents"}
        end)

      assert result == {:ok, "file contents"}

      assert_received {:telemetry_event, [:loom, :tool, :execute, :start], %{system_time: _},
                        %{tool_name: "file_read"}}

      assert_received {:telemetry_event, [:loom, :tool, :execute, :stop], %{duration: _},
    ...
    test "marks success as false on error" do
      ...
      assert_received {:telemetry_event, [:loom, :tool, :execute, :stop], _,
  ...
  describe "emit_session_message/1" do
    test "emits a session message event" do
      LoomTelemetry.emit_session_message(%{
        session_id: "test-session",
        role: :user,
        token_count: 42
      })

      assert_received {:telemetry_event, [:loom, :session, :message], %{system_time: _},
  ...
  describe "emit_decision_logged/1" do
    test "emits a decision logged event" do
      LoomTelemetry.emit_decision_logged(%{
        session_id: "test-session",
        node_type: :decision,
        confidence: 85
      })

      assert_received {:telemetry_event, [:loom, :decision, :logged], %{system_time: _},
